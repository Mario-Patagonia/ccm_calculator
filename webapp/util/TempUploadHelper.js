    ...
    (Constant, BaseObject) => {
        ...
        return BaseObject.extend(
            "zui5ccm.ccmcalculator.util.TempUploadHelper",
            {
                    ...
                    return new Promise((resolve, reject) => {
                        this.oModel.submitChanges({
                            success: (oData) => {
                                ...
                                const iStatusCode = Number(
                                    oData?.__batchResponses?.[0]?.response
                                        ?.statusCode ?? HTTP_SUCCESS,
                                );
                                if (iStatusCode < HTTP_REDIRECT) {
                                    resolve();
                                } else {
                                    reject(
                                        oData?.__batchResponses?.[0]?.response
                 ...
                 */
                _mapData(oRow) {
                    return Object.keys(oMapping).reduce((oCurr, sTargetKey) => {
                        const oKeyMapping = oMapping[sTargetKey];
                        const sSourceKey = oKeyMapping.SourceKeys.find(
                            (sSourceKey) => oRow[sSourceKey],
                        );
                        oCurr[sTargetKey] =
                            oKeyMapping.Converter(oRow[sSourceKey]) ?? "";
                        return oCurr;
                    }, {});
                },
            },
